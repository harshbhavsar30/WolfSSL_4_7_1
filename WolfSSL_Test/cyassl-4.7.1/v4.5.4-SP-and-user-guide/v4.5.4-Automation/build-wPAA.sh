...
#    export CC="aarch64-poky-linux-gcc --host=aarch64-poky-linux --sysroot=$TOOLCHAIN_ROOT/sysroots/aarch64-poky-linux"
    ...
    LOCAL_CPPFLAGS="-DSIZEOF_LONG=8 -DSIZEOF_LONG_LONG=8 -DWOLFSSL_CALLBACKS -DSESSION_CERTS -DSHOW_CERTS -DHAVE_EX_DATA -DWOLFSSL_VERIFY_CB_ALL_CERTS -DWOLFSSL_STATIC_RSA -DWOLFSSL_PUBLIC_MP -DHAVE_FORCE_FIPS_FAILURE -DFP_MAX_BITS=8192" # -DACVP_VECTOR_TESTING"
    ...
    CONFIG_RESULT=$(eval "./configure ${CONFIGURE_FLAGS} --prefix=${INSTALL_LOCATION} --enable-fips=v2 --enable-opensslextra --enable-opensslall --enable-keygen --disable-examples --enable-shared --disable-static --disable-examples --disable-crypttests --enable-sha3 --enable-dsa --enable-armasm --enable-sp=yes,2048,3072,4096 --enable-sp-asm")
...
#    ACVP_LDFLAGS="-L${INSTALL_LOCATION}/lib -Wl,-Map=acvp_harness__output.map"
...
#}
#
#OP_DIR="$WORKING_DIR"/fips/op_test
#INSTALL_LOCATION="${WORKING_DIR}/FIPS-install-dir"
#
#build_and_scp_optest_app(){
#    . "${TOOLCHAIN_ROOT}"/environment-setup-aarch64-poky-linux
...
#    cd "${OP_DIR}" || exit 5
#    OP_CFLAGS="-I${INSTALL_LOCATION}/include"
#    OP_LDFLAGS="-L${INSTALL_LOCATION}/lib -Wl,-Map=optest_output.map"
#    export CFLAGS="${OP_CFLAGS}"
#    export LDFLAGS="${OP_LDFLAGS}"
#    export CPPFLAGS=""
...
#    sshpass -p 'root' scp "${OP_DIR}"/optest root@${DEVICE_IP}:/kernel
#}
